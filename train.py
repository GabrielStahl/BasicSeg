...
import config, data_utils, model_utils, network
...
def main():
    ...
    if not os.path.exists(parent_folder):
        ...
        print("[INFO] Fetching the Oxford IIIT Pet Dataset from cache")
    # define the paths to the images and segmentation maps directories
    images_dir = config.IMAGE_DATASET_PATH
    mask_dir = config.MASK_DATASET_PATH
    ...
    # call the UNet class from the model.py file and initialize the model
    ...
    for epoch in range(1, config.EPOCHS + 1):
        ...
        Path(parent_folder, config.MODEL_CHECKPOINT_PATH).mkdir(parents=True, exist_ok=True)
        state_dict = model.state_dict()
        state_dict["mask_values"] = dataset.mask_values
        # construct the path for saving the checkpoint
        checkpoint_path = os.path.join(
            parent_folder, config.MODEL_CHECKPOINT_PATH, f"checkpoint_epoch{epoch}.pth"
        )
    ...
    model_utils.plot_training_history(parent_folder, epoch_losses, train_scores, val_losses, val_scores)
